...
window.DemoChatroom = function(options: DemoChatroomOptions) {
  ...
  function* demoSaga(
    ...
    let messages = _messages.map((m, i) => ({
      message: m.message,
    ...
    for (let i = -1; i < messages.length; i++) {
      if (i < 0) {
        this.render([], messages[0].username === "bot", resetMessages);
      } else {
        ...
        this.render(messages.slice(0, i + 1), showWaitingBubble, resetMessages);
      }
      yield sleepEffect(delay);
    }
  ...
  this.demo = async (
    ...
    const saga = demoSaga.call(this, messages, resetMessages, delay, keyDelay);
